...
function showCursus(){
  $('#people').hide();
  $('#rooms').hide();
  $("#cursus").load('cursus/cursus.html', function(){
    $("#cursus").show();
  });   
}
...
function presenceCallback(message){
  ...
  if(!roomFlag){
      ...
      if(user_id !== undefined && user_id === message.name){
        ...
        if(message.name === RTCApp.name && message.role !== undefined){
          for(var i =0; i < message.role.length; i ++){
            if(message.role[i] === 'admin' || message.role[i] === 'teacher')
              $('#teacher_id').show();
            if(message.role[i] === 'student')
              $('#student_id').show();
          }
        }
      }       
...
$('#main').delegate('img', 'click', function() {
  event.preventDefault();
  var url = $(this).attr('url_bio');
  if(url !== 'undefined') {
      $("#people_bio").load(url, function(){
          $(".rating").jRating();
      });    
    } 
});
...
function addDataToDiv(data, class_name, class_div, roomFlag){
  ...
  $.each(data, function(key, value) {
    ...
    items.push('<li class="span2"><div class="thumbnail"><img src="' + image + '"'
      +" url_bio=" + bio + ' " alt=""></a> <h4>' + name + '</h4><p><a href="#" class="btn btn-primary ' + 
